...
// runStreamForTest starts stream() in a goroutine against srv and returns a
// channel of every snapshot onSnapshot was called with, plus a channel that
// receives stream()'s return value exactly once, when it returns.
func runStreamForTest(ctx context.Context, srv *testServer) (got chan Snapshot, done chan error) {
	got = make(chan Snapshot, 16)
	done = make(chan error, 1)
	opts := Options{Endpoint: srv.URL, APIKey: "test-key"}.withDefaults()
	go func() {
		done <- stream(ctx, opts, 0, func(s Snapshot) { got <- s })
	}()
	return got, done
}
...
func TestStreamParsesDataFrameIntoSnapshot(t *testing.T) {
	...
	got, _ := runStreamForTest(ctx, srv)
...
func TestStreamSkipsMalformedFrameWithoutDying(t *testing.T) {
	...
	got, done := runStreamForTest(ctx, srv)
	ch := srv.waitForStreamConn(t)
...
func TestStreamReturnsPromptlyOnContextCancel(t *testing.T) {
	...
	_, done := runStreamForTest(ctx, srv)
